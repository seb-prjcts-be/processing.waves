...
static class Waves {
  ...
  // Order matters: pickWaveIndex(seed) returns Math.floor(rng * 34), so the
  // same seed must select the same wave between JS and Java.
  static final WaveDef[] WAVES = new WaveDef[] {
    new WaveDef("classic sine",      "sin(x*.1)*.4",
      (x, t, c) -> (float)(Math.sin(x*0.1) * 0.4),                         "gentle", 62.8319f),
    new WaveDef("sine",              "sin(x*.2)*.25",
      (x, t, c) -> (float)(Math.sin(x*0.2) * 0.25),                        "gentle", 31.4159f),
    new WaveDef("sharp peaks",       "abs(sin(x*.1))*.5",
      (x, t, c) -> (float)(Math.abs(Math.sin(x*0.1)) * 0.5),               "gentle", 31.4159f),
    new WaveDef("square",            "(x*.025)%1 < .5 ? -.5 : .5",
      (x, t, c) -> ((x*0.025f) % 1f) < 0.5f ? -0.5f : 0.5f,                "gentle", 40.0f),
    new WaveDef("pulse",             "(x*.5)%20 < 1 ? -.5 : .5",
      (x, t, c) -> ((x*0.5f) % 20f) < 1f ? -0.5f : 0.5f,                   "gentle", 40.0f),
    new WaveDef("stepped sine",      "ceil(sin(x*.1))*.25",
      (x, t, c) -> (float)(Math.ceil(Math.sin(x*0.1)) * 0.25),             "gentle", 62.8319f),
    new WaveDef("mountain peaks",    "abs(cos(x*.1))*.35 + sin(x*.1)*.25",
      (x, t, c) -> (float)(Math.abs(Math.cos(x*0.1))*0.35 + Math.sin(x*0.1)*0.25), "gentle", 62.8319f),
    new WaveDef("valleys",           "abs(cos(x*.1))*-.35 + sin(x*.1)*.25",
      (x, t, c) -> (float)(Math.abs(Math.cos(x*0.1))*-0.35 + Math.sin(x*0.1)*0.25), "gentle", 62.8319f),
    new WaveDef("zig-zag sine",      "sin(x*.2)*.4 % .12",
      (x, t, c) -> (float)((Math.sin(x*0.2) * 0.4) % 0.12),                "gentle", 31.4159f),
    new WaveDef("batman",            "sin(x*.1)*.7 % .4",
      (x, t, c) -> (float)((Math.sin(x*0.1) * 0.7) % 0.4),                 "gentle", 62.8319f),
    new WaveDef("offset sine",       "ceil(cos(x*.1))*.25 - sin(x*.1)*.25",
      (x, t, c) -> (float)(Math.ceil(Math.cos(x*0.1))*0.25 - Math.sin(x*0.1)*0.25), "gentle", 62.8319f),
    new WaveDef("steps down",        "ceil(tan(x*.1))*.25",
      (x, t, c) -> (float)(Math.ceil(Math.tan(x*0.1)) * 0.25),             "harsh", 31.4159f),
    new WaveDef("steps",             "round(sin(-x*.1))*.25",
      (x, t, c) -> (float)(Math.round(Math.sin(-x*0.1)) * 0.25),           "gentle", 62.8319f),
    new WaveDef("squared sine",      "sq(sin(x*.1))*.25",
      (x, t, c) -> { double s = Math.sin(x*0.1); return (float)(s*s*0.25); }, "gentle", 31.4159f),
    new WaveDef("bumpy sine",        "sin(x*.1)*.25 + sin(x*.5)*.1",
      (x, t, c) -> (float)(Math.sin(x*0.1)*0.25 + Math.sin(x*0.5)*0.1),    "gentle", 62.8319f),
    new WaveDef("wobble sine",       "sin(x*.1)*cos(x*.2)*.5",
      (x, t, c) -> (float)(Math.sin(x*0.1) * Math.cos(x*0.2) * 0.5),       "gentle", 62.8319f),
    new WaveDef("up down noise",     "x*sin(x*.1) % .5",
      (x, t, c) -> (float)((x * Math.sin(x*0.1)) % 0.5),                   "gentle", null),
    new WaveDef("meta sine",         "sin(x*.45 + radians(x))*cos(x*.4)*.5",
      (x, t, c) -> (float)(Math.sin(x*0.45 + Math.toRadians(x)) * Math.cos(x*0.4) * 0.5), "gentle", null),
    new WaveDef("triangle",          "abs((x*.03) % (.5*2) - .5)",
      (x, t, c) -> Math.abs(((x*0.03f) % 1f) - 0.5f),                      "gentle", 33.3333f),
    new WaveDef("ramp",              "-1*(x*.02%1)/1 + 0.5",
      (x, t, c) -> (float)(-1.0 * ((x*0.02) % 1.0) + 0.5),                 "gentle", 50.0f),
    new WaveDef("saw down",          "x*.03 % .5",
      (x, t, c) -> (x*0.03f) % 0.5f,                                       "gentle", 16.6667f),
    new WaveDef("saw up",            "-x*.03 % .5",
      (x, t, c) -> (-x*0.03f) % 0.5f,                                      "gentle", 16.6667f),
    new WaveDef("fade out",          "log(x)*.1",
      (x, t, c) -> (float)(Math.log(x) * 0.1),                             "gentle", null),
    new WaveDef("grow random",       "random(x*.003)",
      (x, t, c) -> c.randomMax(x*0.003f),                                  "harsh", null),
    new WaveDef("noise",             "noise(x*.1) - .5",
      (x, t, c) -> c.noise(x*0.1f) - 0.5f,                                 "harsh", null),
    new WaveDef("fuzzy pulse",       "tan(x*20)*.05",
      (x, t, c) -> (float)(Math.tan(x*20.0) * 0.05),                       "harsh", 17.75f),
    new WaveDef("up down pulse",     "tan(x*.1)*.05",
      (x, t, c) -> (float)(Math.tan(x*0.1) * 0.05),                        "harsh", 31.4159f),
    new WaveDef("bald patch",        "sq(x*.05) % .5",
      (x, t, c) -> (float)(((x*0.05) * (x*0.05)) % 0.5),                   "gentle", null),
    new WaveDef("fuzzy peak sine",   "sin(x*.1) < 0 ? random(-.2, .2) : sin(x*.1)*.5",
      (x, t, c) -> {
        double s = Math.sin(x*0.1);
        return s < 0 ? c.randomRange(-0.2f, 0.2f) : (float)(s * 0.5);
      }, "harsh", null),
    new WaveDef("ramp up sine",      "sin(x)*(x*.01%.5)",
      (x, t, c) -> (float)(Math.sin(x) * ((x*0.01) % 0.5)),                "gentle", null),
    new WaveDef("triangle sine",     "sin(x)*(x*.01%1-.5)",
      (x, t, c) -> (float)(Math.sin(x) * ((x*0.01) % 1 - 0.5)),            "gentle", null),
    new WaveDef("round linked sine", "sin(x*.1)*cos(x*1)*.5",
      (x, t, c) -> (float)(Math.sin(x*0.1) * Math.cos(x) * 0.5),           "gentle", 62.8319f),
    new WaveDef("half sine",         "sin(x*.05)*(x*.1%.5)",
      (x, t, c) -> (float)(Math.sin(x*0.05) * ((x*0.1) % 0.5)),            "gentle", null),
    new WaveDef("smooth solid sine", "sin(x*3.1)*.25",
      (x, t, c) -> (float)(Math.sin(x*3.1) * 0.25),                        "gentle", 2.0268f),
  };
